...
if [ $verbose -eq 0 ]; then
	make clean > /dev/null
	make > /dev/null
	./Application mp1testcases/singlefailure.json > /dev/null
else
	make clean
	make
	./Application mp1testcases/singlefailure.json
fi
...
if [ $verbose -eq 0 ]; then
	make clean > /dev/null
	make > /dev/null
	./Application mp1testcases/multifailure.json > /dev/null
else
	make clean
	make
	./Application mp1testcases/multifailure.json
fi
joincount=`grep joined dbg.log | cut -d" " -f2,4-7 | sort -u | wc -l`
if [ $joincount -eq 100 ]; then
	grade=`expr $grade + 10`
...
if [ $verbose -eq 0 ]; then
	make clean > /dev/null
	make > /dev/null
	./Application mp1testcases/msgdropsinglefailure.json > /dev/null
else
	make clean
	make
	./Application mp1testcases/msgdropsinglefailure.json
fi
joincount=`grep joined dbg.log | cut -d" " -f2,4-7 | sort -u | wc -l`
if [ $joincount -eq 100 ]; then
	grade=`expr $grade + 15`
